...
REGISTER_OP("SparseMergeVal")
    .Input("indices: N * int64")
    .Input("values: N * float")
...
class SparseMergeValOp : public OpKernel {
  public:

  explicit SparseMergeValOp(OpKernelConstruction* ctx) : OpKernel(ctx) {
  ...
  void Compute(OpKernelContext* context) override {
    ...
    auto output_values = output_values_out->vec<float>();
    ...
    for (int i=0; i<shape_dim0; i++)
    {
      ...
      for (int j=0; j<N; j++)
      {
        ...
        auto vals_vec = vals[j].vec<float>();
        ...
        while (index_vec[j] < inds[j].dim_size(0) && indices_vec(2*index_vec[j]) == i)
        {
          ...
          output_values(c) = vals_vec(index_vec[j]);
          index_vec[j]++;
          c++;
        }
...
REGISTER_KERNEL_BUILDER(Name("SparseMergeVal").Device(tensorflow::DEVICE_CPU), SparseMergeValOp);
