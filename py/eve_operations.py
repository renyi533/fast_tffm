...
def eve_rate(curr_target, decay_rate, f_decay_rate, lower, upper, is_local_mode, worker_idx):
  device_str = "/cpu:0"
  var_collection = None
  ...
  with tf.device(device_str), tf.name_scope("eve_scope"):
    ...
    smoothed_f = tf.Variable(0.0, trainable=False, dtype=tf.float32, name="smoothed_f", collections=var_collection)
  ...
  f_decay = tf.convert_to_tensor(f_decay_rate)

  new_f, new_smoothed_f, new_d = eve_alg_op.eve_alg(f, smoothed_f, d, curr_target, step, decay, f_decay, low, up)

  f_assign = tf.assign(f, new_f)
  smoothed_f_assign = tf.assign(smoothed_f, new_smoothed_f)
  step_inc = tf.assign_add(step, 1)
  with tf.control_dependencies([f_assign, step_inc, smoothed_f_assign]):
    d_assign = tf.assign(d, new_d)
  return d_assign, f_assign, smoothed_f_assign
